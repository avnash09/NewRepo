...
subsetDF <- subset(NEI, fips == "24510")[,c("Emissions","type","year")]

DF3agg <- setNames(aggregate(Emissions ~ type+year, data = subsetDF, FUN=sum),c("type","year","Emissions"))

library(ggplot2)

png(file="plot3.png")
g <- ggplot(DF3agg, aes(x=year,y=Emissions, color=type))
